...
- AGR-041: Sprint 2 work-loop-diet (решение владельца 2026-09-06): главный рычаг времени — меньше прогонов тестов: implementer гоняет только тесты своих файлов (список из graph_tests), полная батарея ≤ 2 раз на item через evidence-кэш; context pack передаётся одними байтами и implementer, и verifier/reviewer/judge; агенты обязаны использовать графы (AST graph.json + семантический индекс) — adoption замеряется mb-cost-report.py (graph_share per role), стадии 1 и 5 плана graph-semantic-adoption исполняются внутри Sprint 2 Stage 5, стадии 2–4 остаются там
