...
- AGR-039: mb-context.sh: пофайловый бюджет вывода core-файлов по умолчанию 12288 байт (не 16384 из плана Sprint 1 Stage 3) — решение пользователя 2026-09-05; per-bank тумблер .mb-config context_max_bytes= остаётся для точечной настройки, в банке этого репо не нужен
- AGR-042: Гейт качества программы mb-work-cost-diet (решение владельца 2026-09-06): ускорение и удешевление принимаются только без регресса качества — полная батарея зелёная, verifier PASS с первой попытки, число CRITICAL/WARNING не выше baseline, mb-drift.sh без новых находок; mb-cost-report.py --quality меряет это по транскриптам; правки инструкций в Sprint 3 (тонкий роутер, mb-work-next.sh) проходят A/B на одном и том же item со старыми и новыми инструкциями
- AGR-043: status.md и checklist.md — строгие реестры, ИНФОРМАЦИЯ НЕ УДАЛЯЕТСЯ (уточнение владельца 2026-09-06): status.md = только актуальное состояние проекта, checklist.md = только планы в работе в формате v2 (один блок на план, строка на стадию со статусом); всё сделанное и старое переезжает в progress.md дословно (append-only), жёсткие капы строк контролируют хуки: сверх капа — детерминированная чистка (mb-status-rotate.sh + mb-checklist-prune.sh как компактор/мигратор v2), если не хватило — MB Manager actualize --strict; живые задачи (⬜) не режутся никогда — переполнение живыми планами это сигнал владельцу (exit 3 + список планов: закрыть/поставить на паузу), а не обрезка. Sprint 1: Stage 5 = чеклист v2, Stage 7 = mb-core-cap.sh + Stop-хук + actualize --strict [supersedes AGR-040]
- AGR-044: Графы обязательны для implementer, verifier и reviewer/judge в /mb work (решение владельца 2026-09-06): AST-граф и семантический индекс строятся и поддерживаются автоматически (bootstrap + catchup), покрытие расширяется на bash/bats (regex-экстрактор без новых зависимостей), граф подмешивается в диспатч всех трёх ролей push-моделью (готовый блок ## Graph по Files: item; tooling-core и в verify/review/judge, а не только в implement), nudge-хук работает и в сабагентах, adoption измеряется graph_share в mb-cost-report.py; сторожевой LLM-сабагент не добавляется. План graph-semantic-adoption получает Stages 6–7 и становится пререквизитом cost-diet Sprint 2; Sprint 2 в остальном по AGR-041 (фокусные тесты по graph_tests, полная батарея ≤ 2/item, context pack implementer + проверяющим одними байтами) [supersedes AGR-041]
