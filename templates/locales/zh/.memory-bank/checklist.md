> TODO(i18n-zh): translate this file. See docs/i18n.md.

<!-- checklist.md = open TODO only, one block per plan in flight. Execution detail
     (commit hashes, test counts, closeouts) belongs in progress.md, not here.
     Format: `<!-- mb-plan:<file> -->` + `## <plan title> — k/n` + one
     `- ✅|⬜ Stage N — <name>` line per stage; mb-checklist-prune.sh moves closed
     plans verbatim into progress.md. Hard cap: 100 lines (opt-in SessionEnd
     autoprune: MB_CHECKLIST_AUTOPRUNE=on). -->

# Project — Checklist



## Setup

- ⬜ Define the stack and architecture
- ⬜ Set up CI/CD
- ⬜ Write the first tests
