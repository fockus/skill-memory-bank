...
class NormalizeCache:
    ...
    def __init__(self, mb_path: str | Path | None) -> None:
        ...
        if mb_path is not None:
            self._path = Path(mb_path) / ".index" / "openspec" / "normalize-cache.json"
...
def _validate_slots(raw: dict[str, Any], req: OSRequirement) -> dict[str, Any]:
    ...
    and ``covers`` degrade to "no slot filled" rather than raising.
    """
    ...
    if scenario is not None and (
        not isinstance(scenario, dict) or not scenario.get("name") or not scenario.get("steps")
    ):
        scenario = None
...
def normalize_requirement(
    ...
    *,
    ...
    ``llm`` callable is never invoked. Cache miss: calls ``llm`` with a
    small structured payload; on success the (validated) result is cached
    and returned. On failure (``llm`` raises, or is left ``None`` and no
    real dispatcher is configured) the deterministic slots are returned
    instead, a warning is emitted to stderr, and — deliberately — nothing is
    cached, so a transient LLM outage does not permanently downgrade a
    requirement that could normalize successfully on the next run (REQ-010).
    """
