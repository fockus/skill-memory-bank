...
## Review findings hardening + installer boundaries
- ✅ Security hardening for path, manifest, symlink, and override entrypoints
- ✅ CLI and uninstall contract cleanup
- ✅ Shared Python helper extraction for text and IO operations
- ✅ Adapter framework and manifest schema unification
- ✅ Installer boundary cleanup + adapter-driven uninstall
- ✅ Simplification pass + direct coverage gap closure

---

## Skill v2 — Phase 1 Foundation ✅ (2026-04-22)

### Sprint 1: Rename migration (v1 → v2) ✅
- ✅ `mb-migrate-v2.sh` — APFS-safe + fenced-code-aware idempotent migrator
- ✅ `STATUS→status`, `BACKLOG→backlog`, `RESEARCH→research`, `plan→roadmap`
- ✅ 14 tasks + Contract-First e2e suite + naming guard tests
- ✅ Skill's own `.memory-bank/` dogfooded на v2
- ✅ Review fixes (I1 legacy label, M5 idempotency hardening)
- ✅ Merged to main `b10d154` — 251 passed

### Sprint 2: Autosync + traceability + Phase/Sprint/Task parser ✅
- ✅ `mb-roadmap-sync.sh` — fence-preserving idempotent autosync (6 contract tests)
- ✅ `mb-traceability-gen.sh` — REQ→Plan→Test matrix + zero-spec fallback (6 contract tests)
- ✅ Parser extended: `^#{2,4} (Task|Stage|Phase|Sprint) [0-9]+:` — backward compat
- ✅ Chain wiring в `mb-plan-sync.sh` / `mb-plan-done.sh` (best-effort warn-don't-fail)
- ✅ Slash commands `/mb roadmap-sync` + `/mb traceability-gen`
