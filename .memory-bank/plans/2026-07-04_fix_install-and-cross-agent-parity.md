...
### DoD:
- [ ] rules/commands не затираются без бэкапа; бэкап в манифесте.
- [ ] Non-managed одноимённые файлы — backup/refuse, не тихий clobber.
- [ ] Тесты: RED→GREEN во всех затронутых suites; `shellcheck` clean.
...
### DoD:
- [ ] no-tty без `-y` → быстрый exit с подсказкой (не висит).
- [ ] `-y`/`--yes` в no-tty работает штатно.
- [ ] Тесты: 1 e2e RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] Поведение «global agents always installed» задокументировано (README + doc).
- [ ] Комментарий у call-site объясняет намеренность.
- [ ] Open question про gating зафиксирован.
...
### DoD:
- [ ] `commands/*.md` доставлены в `~/.codex/prompts/` (24 файла).
- [ ] Файлы в манифесте (uninstall их удаляет).
- [ ] Тесты: 1+ bats RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] Dispatchable-агенты (17) в `.opencode/agent/`; partials исключены.
- [ ] Файлы в манифесте.
- [ ] Тесты: 1+ bats RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] Cursor регистрирует `mb-session-end.sh` (CC-совместимый capture).
- [ ] OpenCode plugin вызывает summarize.
- [ ] Тесты: 2 bats RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] Codex получает git-hooks-fallback session-capture в git-репо.
- [ ] Тесты: 1+ bats RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] Команды `/mb` в OpenCode резолвят skill-корень без наличия Claude Code.
- [ ] Тесты: 1+ bats RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] `--agent pi` и `--agent opencode` резолвят команду суб-инвока (exit 0).
- [ ] Help-текст соответствует таблице (нет «Task 13 adds» обещания).
- [ ] Тесты: 2 bats RED→GREEN; `shellcheck scripts/mb-subinvoke-resolve.sh` clean.
...
### DoD:
- [ ] Reviewer-override находится из codex/pi/opencode skill-roots.
- [ ] `MB_SKILLS_ROOT` по-прежнему имеет приоритет.
- [ ] Тесты: 1+ bats RED→GREEN; `shellcheck` clean.
...
### DoD:
- [ ] Код, тесты и доки согласованы на одном контракте регистрации OpenCode-плагина.
- [ ] grep-guard/bats фиксирует выбранный контракт.
- [ ] `shellcheck adapters/opencode.sh` clean.
