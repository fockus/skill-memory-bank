...
## Quality guard (решение владельца 2026-09-06, AGR-042)

Ускорение принимается только без потери качества. Каждая стадия этого спринта, меняющая инструкции (Stage 2 тонкий роутер, Stage 3 `mb-work-next.sh`), проходит **A/B на одном и том же item**: прогон со старыми и с новыми инструкциями, оба измерены `mb-cost-report.py --quality`. Принимается, если: (1) verifier PASS в обоих и число CRITICAL/WARNING находок не выше; (2) полная батарея зелёная; (3) `mb-drift.sh .` без новых находок; (4) стоимость (ходы / output-токены / прогоны тестов) ниже. `mb-cost-report.py --quality` (подпункт Stage 3 этого спринта; TDD: `test_quality_block_counts_verdicts_findings_cycles` на фикстуре с `Verdict: PASS`, 1 CRITICAL, 2 fix-цикла): per session/item — вердикты verifier (`Verdict: PASS|FAIL`), CRITICAL/WARNING, циклы fix (`mb-work-state.sh cycle`), решения judge, `tests_pass` из evidence. A/B-отчёты → `reports/2026-xx-xx_ab-<stage>.md`.

## Gate (plan success criterion)

`commands/mb.md` ≤ 12 KB и `commands/work.md` ≤ 20 KB при зелёных doc-контрактах; блок Agreements в `CLAUDE.md` ≤ 6 KB; один реальный item пройден через `/mb work` с `mb-work-next.sh` на каждом шаге; `install.sh` в песочнице с `hooks.disabled` не возвращает выключенный хук; полная батарея зелёная; `/mb verify` PASS. Phase-гейт (все три спринта): `mb-cost-report.py` на недельном окне после раскатки показывает implementer ≤ 120 ходов avg, ≤ 3 полных прогона тестов на item и старт сессии ≤ 12k токенов — против baseline `reports/2026-09-05_cost-baseline.json`. **Качество (AGR-042):** A/B-отчёты Stage 2 и Stage 3 показывают не худшие метрики качества при меньшей стоимости; `mb-cost-report.py --quality` реализован и покрыт тестом.
