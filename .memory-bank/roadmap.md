...
- [sprint1-review-blockers](plans/2026-09-13_fix_sprint1-review-blockers.md) — fix — sprint1-review-blockers · три воспроизведённых блокера судьи Sprint 1 — progress=0% stages(done=0,in_progress=0,planned=3,total=3)
...
Дальше: `/mb work adapter-parity` (T1 research → T2 offer → T3–T6 hosts → T7 honesty → T8 lifecycle). Не пересекается по файлам с sdd-openspec-parity — тот остаётся параллельным лейном.
...
| install-and-cross-agent-parity | план 2026-07-04 (queued) | частично поглощён `adapter-parity` (AGR-012); остаток сверить на его старте |
...
| cursor/pi-compatibility-remediation | планы 2026-05-24 | Now (cursor); pi-extension-часть поглощена `adapter-parity` (T1/T3/T4) |
