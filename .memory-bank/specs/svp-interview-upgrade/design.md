...
3. **`## Унаследовано…`** — обязательна и обязана стоять до `## Q&A` **только** при
   ...
   Секция обязана быть **ровно одна** независимо от флага → `duplicate_inherited`
   (зеркало `duplicate_qa_section`). Причина — ревью круга 3, находка [22]: две секции
   `## Унаследовано` с противоречащими наборами решений проходили валидацию как `ok`,
   и ни один существующий код C8 не нёс этого смысла. Дубль наследования опаснее дубля Q&A:
   он означает два конфликтующих набора уже принятых решений, из которых генератор молча
   возьмёт один.
