...
**Testing (TDD — tests BEFORE implementation):**
- pytest по контракту команды: фазы 0–10 упорядочены; транскрипт-вход обязателен; candidate-seam присутствует и предшествует записи в specs/; шаг 7 публикует как **draft** (не accepted); шаг 8 **вызывает** `mb-sdd-self-check.sh` и блокирует ready по его exit (draft→ready только по C7 state machine); утверждение «does not auto-generate» удалено.
...
- Врезка в `commands/sdd.md`: шаг 7 зовёт `mb-sdd-candidate.sh publish` (не prompt-`mv`); эскалация D-35 → `discard` + `sdd_status=blocked`.
