...
оценивает **ровно его** (не существующий финальный файл), и только шаг (9) переносит его в
`specs/` — как **draft**, публикация ≠ acceptance (C7). Пока гейт не пройден,
`specs/<topic>/tasks.md` не создаётся и не изменяется (REQ-053).
...
**Дискриминатор записи журнала (нормативно; читают ВСЕ потребители журнала).** В одном
`<topic>.jsonl` живёт больше одного рода записей, поэтому «последняя валидная строка» без указания
рода — не определение, а ловушка: решение C7, дописанное последним, читалось бы как текущий вердикт
ревью. Правило одно и действует для каждого читателя:

- **Отсутствие ключа `kind` означает запись ревью** — вердиктные строки `record` остаются
  **байт-идентичны** прежним: ключ `kind` в них не добавляется ни сейчас, ни впредь. Это не
  удобство, а условие совместимости: S9 (`svp-spec-review-loop` C2) уже спроектирован против
  журнала, где вердикты ревью — строки без дискриминатора, и не меняет подкоманды S2.
- **Любая запись НЕ-ревью несёт `kind`** и обязана быть отфильтрована по нему, а не по эвристике:
  решение C7 — `kind: "decision"` (ниже); S9 добавляет в тот же журнал `kind: "judge"` и
  `kind: "override"` по тому же правилу.
- **«Текущий вердикт»** = последняя валидная строка **без** `kind`. **«Текущее решение»** =
  последняя валидная строка с `kind: "decision"`. Обобщённо: действующее состояние каждого рода —
  последняя валидная строка **этого** рода; строка чужого рода никогда не отвечает за состояние
  своего.
- Читатель, ищущий вердикт по `kind: "review"`, не найдёт **ни одного** — такого литерала в журнале
  нет по построению. Отсутствие ключа и есть признак; это и проверяют тесты обеих половин.

**Решение человека/оркестратора (C7) — отдельный род записи, а не вердикт.** Подрежим
`decide --topic <topic> --attempt <n> --input <path|->` пишет в тот же append-only журнал строку
с закрытой схемой (лишние поля — malformed):

```
{kind: "decision", decision: "accept"|"reject", basis: "skipped"|"dismissed_issues",
 rationale: <непустая строка>, decided_by: <непустая строка>}
```

- Helper добавляет `ts`, `attempt` и `decided_by_provenance: "claimed"` (актор утверждён вызывающим,
  как и `reviewer_provenance` у `record` — ни один потребитель не читает это как верифицированную
  провенанс-запись).
- Решение **не несёт** ключей вердикта (`status`, `verdict`, `reviewer`, `issues`) и не может их
  подменить: `record` и `decide` — два непересекающихся закрытых контракта над одним журналом.
- **`decide` требует предшествующего ревью** (закрывает круг-4 [3]): `basis: "skipped"` допустим
  только когда текущий вердикт — `status: "skipped"`; `basis: "dismissed_issues"` — только когда
  текущий вердикт `CHANGES_REQUESTED` (у APPROVED нечего отклонять, и решение ему не нужно).
  Журнал без единой вердиктной строки → stderr `no_review`, exit 2, ничего не записано; basis,
  противоречащий записанному вердикту → отказ. «Принять, потому что ревью пропущено» при пустом
  журнале — обход гейта, а не решение.
- Exit `decide`: 0 = accept; 1 = reject; 2 = malformed / `no_review` / basis-несовпадение /
  `path_escape`.
- **Containment — общий для обеих подкоманд** (закрывает круг-4 [4]): каталог `tmp/spec-review` и
  файл `<topic>.jsonl` не могут быть симлинками, а realpath финального пути обязан канонически
  лежать под резолвленным банком; иначе `path_escape`, exit 2, без единого байта записи. Правило
  проверяется на **финальном пути записи** и одной реализацией на обе подкоманды: круг 4 нашёл ровно
  то, что бывает, когда его держит только одна из них — `record` отказывал симлинку, а `decide` шёл
  по нему наружу банка.
...
- `red_observed=true` только при `red_match=true`. Отсутствующий или несовпавший red = **FAIL**,
  блокирует implement. **CLI-переключателя, снимающего этот FAIL, нет** (круг-4 [8], измерено на
  этом дереве, а не предположено): `eval-red` отклоняет неизвестный аргумент (`--force`/`--override`
  → exit 2 `unexpected arg`), а `done` отказывает задаче с недоказанным red→green (exit 5
  `no proven red transition`). Указание пользователя продолжить не даёт механического исключения —
  оно записывается в `progress.md` (`mb-work-progress-append.sh`), а задача остаётся открытой, пока
  заявленный red не наблюдён либо Eval не выведен в waiver (только non-gated).
...
шаг (9) кладёт триплет как **draft**, а не как принятый. Ни helper, ни оркестратор не называют
...
- Правки после CHANGES_REQUESTED проходят полный цикл заново **в порядке шагов 7–9**: новый
  ...
  candidate'ом, который ещё не прошёл батарею и ревью (REQ-053). Никаких «частичных» правок в
  принятом файле.
...
До промоушена (шаг 9) и до spec_review конвейер обязан прогнать и пройти **на staged
draft-триплете** `<bank>/tmp/sdd/<topic>/` батарею C8.1–C8.5 — принятый `specs/<topic>/tasks.md`
на этом шаге ещё НЕ тронут (REQ-053: провал батареи не имеет права уничтожить принятый триплет).
Вся батарея исполняется детерминированным
...
4. **Eval preflight** (переписан — закрывает R2-002/R2-009; исполнитель — helper C8a, не оркестратор):
   ...
   - *Поведенческая часть (детерминированный helper `mb-sdd-self-check.sh`, C8a)*: для каждой задачи
     ...
     **Декларация без якорей валидна** (круг-4 [7]): якоря обязательны только на gated-задаче
     (REQ-055, проверяется структурно), а для non-gated red считается по `exit != 0` — ровно как в
     C1. Требовать якорь здесь значило бы отклонять корректную декларацию и блокировать draft→ready;
     от зелёной команды, выдающей себя за red, защищает именно требование ненулевого exit, а не
     наличие якоря.
...
Любой провал (включая хотя бы один `invalid`) оставляет `status: draft`, останавливает конвейер ДО
промоушена, и принятый триплет остаётся byte-identical. `pending_materialization`
...
**Швы ЭТОЙ спеки (объявление, а не шаблон — круг 4, блокер B2).** До ревизии 6 единственное
вхождение `**Seams:**` в этом файле лежало внутри ```-шаблона выше, то есть спека **показывала
форму и не объявляла собственный шов** — истинно с ревизии 2, и валидатор был прав: он намеренно
пропускает fenced-блоки (`mb_spec_validate_v2.py:229`). Реальное объявление:

**Seams:**
- CLI-поверхность детерминированных helper'ов зоны — `stdout` в виде `key=value` строк плюс
  exit-код, вызов процессом из bats (`mb-sdd-self-check.sh`, `mb-sdd-candidate.sh`,
  `mb-sdd-review-result.sh`, `mb-work-state.sh eval-red|eval-green`, `mb-estimate-check.sh`,
  `mb-spec-validate.sh`)
- Python-API парсера — `mb_work_items.parse_work_items()`, вызываемый импортом из pytest: проекция
  pre-v2 ключей C2 утверждается о структуре JSON, а не о строках stdout
- Текст prompt-файлов как артефакт — контракт `commands/sdd.md` / `commands/work.md`, утверждаемый
  pytest/bats поверх самого файла
**Seam rationale:** три шва, потому что спека охватывает три РАЗНЫХ вида артефакта (shell-CLI, python-модуль, prompt-текст), и каждый проверяется на СВОЁМ уже существующем шве репозитория — ни один не введён этой спекой (D-17: существующие > новые). Сводить их к одному пришлось бы через обёртку, которой в репозитории нет. Честная граница третьего шва: grep по prompt-файлу доказывает, что фраза есть, а не что прогон её исполняет — именно поэтому механические части (перенос candidate, вердикт ревью, батарея) вынесены из промпта в helper'ы C4a/C5/C8a и проверяются на первом шве.
