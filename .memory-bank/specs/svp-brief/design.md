...
- **Семантика политики `brief-input`** зафиксирована S1-C5 и потребляется как есть: `<private>`
  находку **не** подавляет; `<!-- mb-secret-ok -->` на строке-находке или на строке непосредственно
  над ней подавляет только эту находку; бинарный/нечитаемый/неподдерживаемого типа файл →
