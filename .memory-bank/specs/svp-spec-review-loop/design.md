...
- `status [--mb PATH] <topic>` — печатает ровно одну строку
  ...
  **Решение судьи учитывается, только пока оно относится к действующему вердикту (AMEND-S9-5):**
  строка `kind: "judge"`, чей `attempt` меньше `attempt` действующего вердикта, — история, а не
  состояние; поле печатается как `judge=none`. Буквальное «последняя валидная строка каждого kind»
  здесь даёт fail-open: GO на попытке 1 → fix-проход → свежий независимый review (ADR-S9-2)
  записывает CHANGES_REQUESTED на попытке 2 → `status` продолжает отвечать `judge=GO`, и гейт C5,
  чьё правило — «CHANGES_REQUESTED без judge GO ⇒ blocked» (REQ-009), пускает работу по вердикту,
  которого никто не судил. Сужение может только сказать «судьи не было» и не может выдумать GO —
  это единственное направление ошибки, допустимое в гейте. REQ-009 спрашивает про **этот** вердикт,
  поэтому реализовано требование, а поправки требовала формулировка дизайна.

Дискриминатор записи — нормативно у владельца журнала (`svp-sdd-core` design C5): **отсутствие
ключа `kind` = запись ревью**, любая не-ревью запись несёт `kind` (`decision` — решение C7,
`judge` и `override` — S9). Читатель, ищущий вердикт по `kind: "review"`, не найдёт ни одного:
такого литерала в журнале нет по построению. Значение `kind`, неизвестное читателю, — не вердикт,
а нечитаемая строка (иначе будущий род записи молча отвечал бы за состояние ревью).

Журнал остаётся append-only: строки никогда не редактируются и не удаляются; «действующее
состояние» = последняя валидная строка **этого же** kind (правило S2-C5 сохраняется).
...
- **ADR-S9-9. Решение судьи привязано к вердикту, который оно судило (AMEND-S9-5).** Context:
  реализация C2 вскрыла, что независимый выбор «последней строки каждого kind» позволяет решению
  с попытки 1 отвечать за вердикт попытки 2 — то есть гейт получает пропуск, которого никто не
  выдавал; это тот же класс, что AMEND-S9-1, только вход не «не смог прочитать», а «прочитал не
  про то». Decision: judge-строка действует, пока её `attempt` совпадает с `attempt` действующего
  вердикта. Consequences: после fix-прохода спека снова непринята до нового решения судьи —
  дороже на один вызов судьи, но гейт перестаёт пропускать несудимые вердикты.
