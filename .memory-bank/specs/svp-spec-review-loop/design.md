...
#### Реестр писателей и читателей журнала (сверять при КАЖДОЙ находке)

Правило судьи: закрывая находку по журналу, проверь её на **всех** писателях и **всех** читателях,
а не на том одном, кого тронул фикс. Правило работает, только если список существует как артефакт —
по памяти он уже трижды проверялся неверно (круг 4 [4]: containment был на `record` и отсутствовал
на `decide`; B4: ростер был на `judge` и отсутствовал на `record`). Список ведётся здесь.

| Писатель | Подкоманда | Называет модель? | Под ростером | Через общий `append()` |
|---|---|---|---|---|
| вердикт ревью | `record` (без `--kind`) | да (`reviewer.model`) | да (`sdd.spec_review.model`) | да |
| решение C7 | `decide` | нет | н/п | да |
| решение судьи | `record --kind judge` | да (`judge_model`) | да (`sdd.spec_judge.model`) | да |
| override гейта | `record --kind override` | нет | н/п | да |

| Читатель | Что читает | Как |
|---|---|---|
| `status` (C2) | последнюю валидную строку каждого kind | единственный парсер журнала |
| work-гейт (C5) | `spec_review` / `judge` / `override` | ТОЛЬКО через `status`, JSONL сам не парсит (ADR-S9-4) |
| `record --kind judge` | действующий вердикт (findings, `attempt`, модель ревьюера) | тот же парсер |
| `decide` | действующий вердикт (правило basis, C5 круга 4 [3]) | тот же парсер |

Инварианты, которые этот реестр удерживает: **ростер — на каждом писателе, называющем модель**;
**containment и append-mode — одной реализацией на всех писателей**; **грамматика дискриминатора —
одним классификатором на всех читателей**. Новый писатель или читатель добавляется в таблицу в том
же коммите, что и код, иначе следующая находка снова будет закрыта на одной половине.
