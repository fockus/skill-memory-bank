...
### I-208 — [review S1 judge] .memory-bank/plans/done/2026-09-05_fix_mb-work-cost-diet-sprint1.md:291 — Full test battery is not green; Sprint 1's Gate wording requires tests_pass: true [HIGH, DONE 2026-09-17, 2026-09-13] — план fix_i208-test-battery, 9 стадий, verifier PASS по каждой. Было (0c0e50a): pytest 12 failed / 2550 passed / 26 skipped; bats 13 падений + 10 тестов не исполнялись (Executed 3356 вместо 3366). Стало: pytest 2577 passed / 11 skipped / 0 failed; bats 3376 исполнено = bats --count 3376, 0 not ok, 0 warnings. Двенадцать корней A…M: три бага продукта (eval_declaration после aaa1b2b; valid_topic зависел от collation локали и пропускал topic 'Foo', из-за чего candidate с секретом оставался на диске; mb-lint-run.sh и mb-test-run.sh читали раскрашенный вывод и рапортовали ok=true / tests_pass=null при реальных ошибках), пробел в commands/mb.md, кириллица в именах bats-тестов, необъявленная dev-зависимость networkx, четыре бага харнеса, три устаревших теста
...
### I-214 — spec tasks.md с ошибочными mb-stage-маркерами сертифицируется как «нечего гейтить» (NOFILE rc 0) вместо NOITEM — четвёртая форма битой поверхности декларации, не закрытая ни I-196, ни Stage 2 плана i208-test-battery: по имени и содержимому легаси-план в work/tasks.md и спека с перепутанными маркерами неразличимы [MED, NEW, 2026-09-17]

### I-215 — В bats-тестах этого репозитория падение [[ ]] в СЕРЕДИНЕ тела не роняет тест — только в последней позиции (проверено зондом: '[[ abc == *zzz* ]]; true' даёт ok, '[ abc = zzz ]; true' даёт not ok). Тот же класс, что I-147 для '! cmd': любая проверка [[ ]] не в последней строке декоративна. Нужен линтер/контракт по образцу tests/pytest/test_bats_assertion_contract.py, расширенный на [[ ]], и ревизия существующих [HIGH, NEW, 2026-09-17]

### I-216 — mb-glossary.sh:96 гасит 2>/dev/null у mb_lock_acquire и клеит ярлык error=lock_timeout на ЛЮБОЙ ненулевой код, включая lock_usage/lock_corrupt: под 4-кратной параллельной нагрузкой тест 'with an adequate lock timeout every concurrent upsert succeeds' падает в 10 из 24 прогонов, рейсеры выходят rc=2 через секунды, а не через 90с таймаута — реальная причина, вероятно, fail-closed ветка [ -L ] || [ ! -d ] в _lib.sh:910 в окне между mkdir и -d. Диагностика, которая называет таймаутом не таймаут, — тот же класс врущего харнеса, ради которого делался i208 [HIGH, NEW, 2026-09-17]
