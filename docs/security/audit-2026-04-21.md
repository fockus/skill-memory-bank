...
> **Archived historical snapshot.** This audit predates the 4.x/5.x releases; the key
> findings (uninstall path validation, manifest handling, `metrics.sh` override execution)
> were addressed afterwards — see [SECURITY.md](../../SECURITY.md) § Known security-relevant
> design decisions. Kept for transparency and regression context.
