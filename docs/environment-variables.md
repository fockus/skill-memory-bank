...
| `MB_CONTEXT_MAX_BYTES` | Per-file byte cap for the core files printed by `mb-context.sh` (`/mb context`, `/mb start`). `0` disables the cap, as does `--full`. Overrides `context_max_bytes=` in `<bank>/.mb-config`. | `16384` |
