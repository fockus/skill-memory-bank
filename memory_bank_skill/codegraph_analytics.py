...
# PageRank scores keep full precision in ``codegraph_rank``; rounding is display-only.
# 6dp keeps small scores (mean PR ≈ 1e-4 on large graphs) distinguishable in the table.
_PAGERANK_DECIMALS = 6
...
def compute_degree(graph: dict[str, Any]) -> dict[str, int]:
    ...
    Out-degree is attributed to the edge source (last ``:`` segment). In-degree
...
# ═══════════════════════════════════════════════════════════════
# PageRank (symbol-level, optional networkx)
# ═══════════════════════════════════════════════════════════════


def compute_pagerank(graph: dict[str, Any]) -> dict[str, float] | None:
    """Symbol-level PageRank — see ``codegraph_rank.compute_pagerank``.

    Thin façade so the analytics module stays under the 400-line gate while
    keeping the public surface (``cga.compute_pagerank``) stable. This is the
    single networkx gate for ranking: ``codegraph_rank`` is pure math, so the
    ``HAS_NETWORKX`` check lives here (tests monkeypatch it on this module)
    before delegating to the pure-stdlib implementation.
    """
    if not HAS_NETWORKX:
        return None
    return _cgrank.compute_pagerank(graph)
...
def split_god_nodes(
    ...
    """Rank nodes, split into ``modules`` and ``symbols`` (functions / classes).

    When ``pagerank`` is supplied the primary key is PageRank descending (degree
    as the secondary tie-break, then name); otherwise it falls back to degree
    descending (name tie-break). Both orders are deterministic. Without the
    module/symbol split, module-level hubs (test files accumulate every
    import/call) drown the real abstractions; separating them surfaces both. Each
    list is capped at ``top_n``; rows carry ``pagerank`` only when it is known.
    """
    ...
    if pagerank is not None:
        ranked = sorted(
            degree.items(),
            key=lambda kv: (-pr.get(kv[0], 0.0), -kv[1], kv[0]),
        )
    else:
...
def _pr_cell(row: dict[str, Any]) -> str:
    """Render the PageRank score column with fixed precision for stable diffs."""
    return f"{row['pagerank']:.{_PAGERANK_DECIMALS}f}"
...
def render_god_nodes_md(
    ...
    With ``pagerank`` (networkx available) the Top symbols / Top modules tables
    ...
    has_pr = pagerank is not None

    rank_note = (
        "Symbols and modules are ranked by **PageRank** "
        "(transitive importance); degree is shown as a secondary signal."
        if has_pr
        else "Symbols and modules are ranked by **degree**."
    )
    lines = [
        "# God nodes & code-graph analytics",
        "",
        "Automatically generated by `mb-codegraph.py`. " + rank_note + " "
        "High-rank nodes are decomposition candidates; bridge files and "
        "low-cohesion communities are refactoring hotspots.",
        "",
    ...
    if has_pr:
        lines += _md_table(
            ...
            [
                [i, f"`{r['name']}`", _pr_cell(r), r["degree"]]
                for i, r in enumerate(split["modules"], 1)
            ],
        )
    else:
        lines += _md_table(
