...
#   - ~/.cursor/hooks.json + ~/.cursor/hooks/*.sh with twelve _mb_owned bindings
...
@test "cursor-global: install writes ~/.cursor/hooks.json with twelve _mb_owned entries" {
  bash "$REPO_ROOT/install.sh" >/dev/null
...
@test "cursor-global: install is idempotent — two runs keep exactly twelve _mb_owned entries" {
  bash "$REPO_ROOT/install.sh" >/dev/null
  bash "$REPO_ROOT/install.sh" >/dev/null
