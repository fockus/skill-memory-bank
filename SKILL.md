...
| `mb-checklist-autoprune.sh` | SessionEnd | Opt-in (`MB_CHECKLIST_AUTOPRUNE=on`, default off) compaction of a `checklist.md` past the cap (`MB_CHECKLIST_CAP_LINES` / `MB_CHECKLIST_MAX_LINES` / `.mb-config` `checklist_max_lines=` / 100) via `mb-checklist-prune.sh --apply`, under a lock, fail-safe |
