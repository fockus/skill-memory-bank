...
5. **Rotate status** — `bash ~/.claude/skills/memory-bank/scripts/mb-status-rotate.sh --apply --mb .memory-bank` — moves dated `## ` sections of `status.md` past the newest 3 into `progress.md` as `## [status archive] …` blocks. Undated sections are never moved; aborts without touching `status.md` if an append does not land. Idempotent.
